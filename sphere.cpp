...
float predictwall(s1){
  float a = pow(s1.velocity.at(0),2)+ pow(s1.velocity.at(1),2)+ pow(s1.velocity.at(2)-s2.velocity.at(2),2);
...
vector<float> changev(spheres s1,spheres s2){
  ...
  vector<float> newvelocity = subv(s1.velocity,mulv(dotproduct(v,c)*m/(dotproduct(c,c)),c));
  return newvelocity;
}
...
vector<float> wallchange(spheres s1){
  ...
  vertor<float> sproject = mulv(mulv(s1.velocity,unit),unit);
  std::vector<float> soth = subv(s1.velocity,sproject);
  newvelocity = subv(soth,sproject);
  return newvelocity;
}
...
int main(int argc,char** argv){
  ...
  for(int i = 0; i != spheresname.size(); i++){
    cout << spheresname.at(i).name << 'm =' << spheresname.at(i).mass << 'r ='<< spheresname.at(i).radius <<'p ='<<spheresname.at(i).position<<'v ='<<spheresname.at(i).velocity<<endl;
  }
  cout <<'energy' << energy;
  ...
  while (spheresname.size()>0){
    ...
    if mintime < wall{
      ...
      if spheresname.at(sphere2).count !=0 || spheresname.at(sphere1).count != 0{
        cout << 'time of event' << timesum << endl;
        cout <<'colliding' << spheresname.at(sphere1).name << spheresname.at(sphere2).name;
        cout << spheresname.at(sphere1).name << 'm =' << spheresname.at(sphere1).mass << 'r ='<< spheresname.at(sphere1).radius <<'p ='<<spheresname.at(sphere1).position<<'v ='<<spheresname.at(sphere1).velocity<<endl;
        cout << spheresname.at(sphere2).name << 'm =' << spheresname.at(sphere2).mass << 'r ='<< spheresname.at(sphere2).radius <<'p ='<<spheresname.at(sphere2).position<<'v ='<<spheresname.at(sphere2).velocity<<endl;
        cout <<'energy' << energy;
        cout << 'momentum' << momentum;
      }
    }
      ...
      if spheresname.at(sphere0).count != 0{
        ...
        cout << spheresname.at(sphere0).name << 'm =' << spheresname.at(sphere0).mass << 'r ='<< spheresname.at(sphere0).radius <<'p ='<<spheresname.at(sphere0).position<<'v ='<<spheresname.at(sphere0).velocity<<endl;
        cout <<'energy' << energy;
        cout << 'momentum' << momentum;
      }
    for(int i = 0; i < spheresname.size();i++){
